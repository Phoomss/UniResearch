...
export default async function ReviewWorkspace({params}:{params:Promise<{id:string}>}){
  ...
  return <DashboardShell active="02"><main className="dash-main"><p className="eyebrow">[ Advisor / administrator workspace ]</p><h1 className="title">Research review</h1>
    {!research.ok?<StatePanel kind="error" title="Submission context unavailable" detail={research.error.message}/>:(()=>{const item=adaptResearch(research.data,categories.ok?categories.data:[]);return <div className="review-workspace"><article className="panel review-research"><div className="folio-heading"><ArchiveTab>{item.category}</ArchiveTab><span className="mono muted">{item.ref}</span></div><h2 className="section-title">{item.titleTh}</h2><p className="latin">{item.titleEn}</p><Status tone={item.status==="approved"?"approved":item.status==="rejected"?"error":"review"}>{item.statusRaw}</Status><section><h3>Abstract</h3><AbstractRenderer abstract={item.abstract}/></section><dl className="metadata-list"><div><dt>Department</dt><dd>{item.department}</dd></div><div><dt>Year</dt><dd>{item.year}</dd></div></dl><ResearchActions researchId={item.id} hasDocument={item.hasDocument} authenticated/></article><ReviewForm researchId={item.id}/></div>})()}
  </main></DashboardShell>;
}
