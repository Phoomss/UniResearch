...
export default async function AdminResearchPage() {
  ...
  return <main className="admin-main"><header className="admin-page-heading admin-heading-actions"><div><p>คลังผลงานวิจัย <span>ระบบจัดการข้อมูล</span></p><h1>จัดการผลงานวิจัย</h1></div><div><Link className="admin-primary-action" href="/student/research/new"><Plus size={17} />เพิ่มผลงานวิจัยใหม่</Link><button className="admin-secondary-action admin-export-action" type="button" disabled title="ระบบส่วนหลังยังไม่มีระบบส่งออกข้อมูล"><Download size={17} />ส่งออกข้อมูล</button></div></header><AdminResearchManager records={records} /></main>;
}
