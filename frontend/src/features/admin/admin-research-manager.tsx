...
export function AdminResearchManager({
  records,
}: {
  records: AdminResearchRecord[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((item) => {
      const matchesQuery =
        !keyword ||
        [item.title, item.titleEn, item.author, item.ref]
          .join(" ")
  ...
  return (
    <>
        ...
        <div className="admin-filter-grid">
          <label className="admin-search-field">
            ...
            <input
              ...
              placeholder="ค้นหาชื่องานวิจัย, ผู้เขียน หรือรหัส"
            />
          </label>
      ...
      <section className="admin-table-card">
        <div className="admin-research-table admin-table-head">
          <span>รายละเอียดผลงาน</span>
          <span>หมวดหมู่ &amp; ปีการศึกษา</span>
          <span>สถานะ</span>
          <span>ปรับปรุงเมื่อ</span>
          <span>ดำเนินการ</span>
        </div>
        ...
        {filtered.length ? (
          filtered.map((item) => (
            <article
              className="admin-research-table admin-table-row"
              key={item.id}
            >
              <div className="admin-research-title-cell">
                <span
                  className={`admin-document-mark status-${item.status}`}
                >
                  {item.ref.slice(0, 2)}
                </span>
                <div>
                  ...
                  <small className="mono">[อ้างอิง: {item.ref}]</small>
                </div>
              </div>

              <div>
                <span className="admin-category-tag">{item.category}</span>
                <small>ปี {item.year}</small>
              </div>
          ...
          <div className="admin-empty-row">
            ไม่พบผลงานที่ตรงกับตัวกรอง
          </div>
        )}

        <footer className="admin-table-footer">
          <span>
            แสดง {filtered.length} จาก {records.length} รายการผลงานวิจัย
          </span>
          <div>
            <button disabled>‹</button>
            <button className="active">1</button>
            <button disabled={filtered.length < 4}>2</button>
            <button disabled>›</button>
          </div>
        </footer>
      </section>
